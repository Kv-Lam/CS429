from transformers import (GPT2LMHeadModel,GPT2Tokenizer,DataCollatorForLanguageModeling,Trainer,TrainingArguments,TrainerCallback)
...
class ChordDataset(Dataset):
    # Tokenizes a flat text file and chunks it into fixed-size blocks for language modeling
...
class LossPlotCallback(TrainerCallback):
    # Records train/eval loss each epoch and saves loss and perplexity plots at training end
    ...
    def _plot(self):
        ...
        # Loss per epoch
        ax = axes[0]
        ax.plot(train_epochs, train_losses, color="#2563eb", linewidth=2, marker="o", markersize=6, label="Train loss")
        ax.plot(eval_epochs, eval_losses, color="#dc2626", linewidth=2, marker="s", markersize=6, label="Eval loss")
...
def fine_tune(dataset_path, output_name):
    # Loads GPT-2, registers special tokens, fine-tunes on the given dataset, and pushes to Hugging Face
    tokenizer = GPT2Tokenizer.from_pretrained("gpt2")
    tokenizer.pad_token = tokenizer.eos_token
    tokenizer.add_special_tokens({"additional_special_tokens": ["<|title|>", "<|key|>", "<|chords|>"]})
    ...
    training_args = TrainingArguments(output_dir=f"./results/{output_name}", num_train_epochs=5, per_device_train_batch_size=8, gradient_accumulation_steps=4, eval_strategy="epoch", save_strategy="epoch", save_total_limit=2, logging_steps=10, fp16=torch.cuda.is_available(), load_best_model_at_end=True,)

    trainer = Trainer(model=model, args=training_args, data_collator=collator, train_dataset=train_dataset, eval_dataset=eval_dataset, callbacks=[LossPlotCallback(output_name)],)
...
#fine_tune("train_abc.txt", "gpt2-abc")