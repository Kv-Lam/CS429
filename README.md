...
> **Disclaimer:** The `results.ipynb` notebook, `evaluations.py`, and `harte_to_abc_dict.py` were developed with assistance from Claude (claude.ai/code). `results.ipynb` used Claude to assist with parsing generated output, loading the datasets, scaling the prompts to 250, and making the evaluation loop. `harte_to_abc_dict.py` used Claude to help with the translation dictionary mapping and human-fixed/verified. Claude was used in `convert_chords.py` to assist with research and converting the JSON files to TXT. Claude was also used in `train.py` to assist with writing the graphing functions to evaluate how well the model was learning.
